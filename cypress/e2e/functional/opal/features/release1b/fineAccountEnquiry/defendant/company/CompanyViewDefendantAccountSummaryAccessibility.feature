...
@JIRA-NFR:PO-2322
Feature: Company View Defendant Account Summary Accessibility
  ...
  Scenario: Company account details reflows without horizontal overflow at narrow viewport
    Given I create a "company" draft account with the following details and set status "Publishing Pending" using user "opal-test-10@dev.platform.hmcts.net":
      | Account_status                      | Submitted                                          |
      | account.defendant.company_name      | A very long company name that should wrap cleanly. |
      | account.defendant.email_address_1   | Accdetailcomp{uniq}@test.com                       |
      | account.defendant.post_code         | AB23 4RN                                           |
      | account.account_type                | Fine                                               |
      | account.prosecutor_case_reference   | PCR-AUTO-003                                       |
      ...
      | account.collection_order_made_today | false                                              |
      | account.payment_card_request        | false                                              |
    When I open the company account details for "A very long company name that should wrap cleanly."
    ...
    And I should see the page header contains "A very long company name that should wrap cleanly."
