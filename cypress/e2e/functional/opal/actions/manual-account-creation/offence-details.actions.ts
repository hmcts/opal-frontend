...
export class ManualOffenceDetailsActions {
    ...
    summaryRoot.within(() => {
      Object.entries(expectations).forEach(([label, value]) => {
        ...
        if (label === 'Minor creditor') {
          cy.get('h4.govuk-summary-card__title', this.common.getTimeoutOptions()).should('contain.text', value);
          return;
        }
  ...
  assertStandaloneMinorCreditorDetails(
    ...
    Object.entries(expectations).forEach(([label, value]) => {
      ...
      if (label === 'Minor creditor') {
        summary.find('h4.govuk-summary-card__title').should('contain.text', value);
        return;
      }
