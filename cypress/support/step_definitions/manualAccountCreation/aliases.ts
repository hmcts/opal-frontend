...
When('I remove the {string}, {string} to be cleared', (alias: string, aliasField: string) => {
  cy.contains('legend', alias).siblings().contains('lib-govuk-text-input', aliasField).find('input').clear();
});
Then('I see the text box {string} below the sub heading {string}', (aliasField: string, alias: string) => {
  cy.contains('legend', alias).siblings().contains('lib-govuk-text-input', aliasField).should('exist', aliasField);
});
...
When('I select add another alias', () => {
  cy.get('[id*="_details_add_alias-conditional"] > lib-govuk-button > button').click();
});
