...
/**
 * @step Assert that the service name in the global header contains specific text.
 * @param expectedHeader – Text expected in the service name link (e.g., "Opal").
 *
 * @example
...
Then('I should see the service header containing text {string}', (expectedHeader: string) => {
  const header = applyUniqPlaceholder(expectedHeader);
  log('assert', `Verify service header contains '${header}'`, { expectedHeader: header });

  Common().assertServiceHeaderContains(header);
});
