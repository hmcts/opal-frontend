...
/**
 * @step Assert a result code text is absent from imposition rows.
 * @description Checks imposition result code inputs rather than the whole page.
 *
 * @param text - Result code text expected to be absent.
 * @example And I do not see "Compensation (FCOMP)" in the imposition result code fields
 */
Then('I do not see {string} in the imposition result code fields', (text: string) => {
  log('assert', 'Asserting result code is not listed in impositions', { text });
  offenceDetails().assertResultCodeAbsentInImpositions(text);
});
...
/**
 * @step Assert the Remove imposition link is not visible for an imposition.
 * @description Assert the Remove imposition link is not visible for an imposition.
 *
 * @param imposition - 1-based imposition number.
 * @example And I do not see the "Remove imposition" link for imposition 1
 */
Then('I do not see the "Remove imposition" link for imposition {int}', (imposition: number) => {
  log('assert', 'Asserting remove imposition link is not visible', { imposition });
  flow().assertRemoveImpositionLinks([imposition], false);
});
