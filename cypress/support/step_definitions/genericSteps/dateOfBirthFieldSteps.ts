...
Then('I see {string} in the Date of birth field', (dob: string) => {
  cy.get('opal-lib-moj-date-picker[labeltext="Date of birth"]').find('input').should('have.value', dob);
});
...
Then('I see {string} in the date of birth panel', (expectedText: string) => {
  cy.get('opal-lib-moj-ticket-panel').find('p').should('contain.text', expectedText);
});
