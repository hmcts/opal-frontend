...
//   // Make the function accept an optional mock parameter
//   const setupComponent = (mockData = FINES_MAC_STATE_MOCK) => {
...
//   it.only('(AC4) Form can be successfully submitted with all required fields completed', { tags: ['@PO-857'] }, () => {
//     setupComponent();
...
//     cy.get(DOM_ELEMENTS.addressLine2Input).type(FINES_FIXED_PENALTY_MOCK.validAddressLine2);
...
//     cy.get(DOM_ELEMENTS.vehicleRadioButton).click(); // Ensure vehicle option is selected
...
//     cy.get(DOM_ELEMENTS.timeOfOffenceInput).type(FINES_FIXED_PENALTY_MOCK.validTimeOfOffence);
