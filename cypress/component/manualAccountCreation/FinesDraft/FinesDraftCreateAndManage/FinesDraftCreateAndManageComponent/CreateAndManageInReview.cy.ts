...
describe('FinesDraftCreateAndManageInReviewComponent', () => {
  ...
  it('(AC.1) render all the fields In review account', { tags: buildTags('@JIRA-STORY:PO-584') }, () => {
  ...
  it(
    ...
    { tags: buildTags('@JIRA-STORY:PO-584') },
    () => {
      interceptGetRejectedAccounts(200, { count: 0, summaries: [] });
  ...
  it('AC.3 verify the table of headers in review tab', { tags: buildTags('@JIRA-STORY:PO-584') }, () => {
    const inReviewAccountsMockData = structuredClone(OPAL_FINES_DRAFT_ACCOUNTS_MOCK);
  ...
  it('(AC.4a) The table should have the correct default ordering', { tags: buildTags('@JIRA-STORY:PO-584') }, () => {
    const inReviewAccountsMockData = structuredClone(OPAL_FINES_DRAFT_ACCOUNTS_MOCK);
