export const FIXED_PENALTY_DEFENDANT_MOCK = {
  draft_account_id: 1003,
  business_unit_id: 77,
  created_at: '2025-11-21T18:20:52.998753Z',
  submitted_by: 'L077JG',
  submitted_by_name: 'opal-test',
  account: {
    account_type: 'Fixed Penalty',
    defendant_type: 'adultOrYouthOnly',
    originator_name: 'Fixed Penalty Office',
    originator_id: 36,
    prosecutor_case_reference: null,
    enforcement_court_id: 770000000001,
    collection_order_made: null,
    collection_order_made_today: null,
    collection_order_date: null,
    suspended_committal_date: null,
    payment_card_request: null,
    account_sentence_date: '2025-11-06',
    defendant: {
      company_flag: false,
      title: 'Mr',
      surname: 'FAKELAST',
      forenames: 'FakeFixed',
      company_name: null,
      dob: '2005-11-02',
      address_line_1: '63 Fake Street',
      address_line_2: null,
      address_line_3: null,
      address_line_4: null,
      address_line_5: null,
      post_code: 'AB12FD',
      telephone_number_home: null,
      telephone_number_business: null,
      telephone_number_mobile: null,
      email_address_1: null,
      email_address_2: null,
      national_insurance_number: null,
      driving_licence_number: null,
      pnc_id: null,
      nationality_1: null,
      nationality_2: null,
      ethnicity_self_defined: null,
      ethnicity_observed: null,
      cro_number: null,
      occupation: null,
      gender: null,
      custody_status: null,
      prison_number: null,
      interpreter_lang: null,
      debtor_detail: {
        vehicle_make: null,
        vehicle_registration_mark: null,
        document_language: null,
        hearing_language: null,
        employee_reference: null,
        employer_company_name: null,
        employer_address_line_1: null,
        employer_address_line_2: null,
        employer_address_line_3: null,
        employer_address_line_4: null,
        employer_address_line_5: null,
        employer_post_code: null,
        employer_email_address: null,
        employer_telephone_number: null,
        aliases: null,
      },
      parent_guardian: null,
    },
    offences: [
      {
        offence_type: 'Vehicle',
        date_of_sentence: '2025-11-06',
        imposing_court_id: null,
        offence_id: 33369,
        impositions: [
          {
            result_id: 'FO',
            amount_imposed: 100,
            amount_paid: 0,
            major_creditor_id: null,
            minor_creditor: null,
          },
        ],
      },
    ],
    fp_ticket_detail: {
      notice_number: '12344',
      date_of_issue: null,
      time_of_issue: '10:15',
      fp_registration_number: 'AB12CD',
      notice_to_owner_hirer: null,
      place_of_offence: 'Fake Street',
      fp_driving_licence_number: 'ABCDE123456AA1B1',
    },
    payment_terms: {
      payment_terms_type_code: 'B',
      effective_date: null,
      instalment_period: null,
      lump_sum_amount: null,
      instalment_amount: null,
      default_days_in_jail: null,
      enforcements: null,
    },
    account_notes: [
      { note_type: 'AC', account_note_text: 'Defendant scenario account comment', account_note_serial: 3 },
      { note_type: 'AA', account_note_text: 'Defendant scenario account note', account_note_serial: 2 },
    ],
  },
  account_snapshot: {
    account_type: 'Fixed Penalty',
    created_date: '2025-11-21T18:20:52.998753344Z',
    submitted_by: 'L077JG',
    defendant_name: 'FakeFixed FAKELAST',
    submitted_by_name: 'opal-test',
    business_unit_name: 'Camberwell Green',
  },
  account_type: 'Fixed Penalty',
  account_status: 'Submitted',
  account_status_message: null,
  account_status_date: '2025-11-21T18:20:53.035557Z',
  timeline_data: [
    {
      username: 'opal-test',
      status: 'Submitted',
      status_date: '2025-11-20',
      reason_text: null,
    },
  ],
  version: '0',
};
