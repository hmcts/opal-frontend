...
const DASHBOARD_JIRA_LABEL = '@JIRA-LABEL:dashboard';

const buildTags = (...tags: string[]) => [...tags, DASHBOARD_JIRA_LABEL];
...
describe('DashboardComponent', () => {
  ...
  it.skip(
    'should only show CAV when user has correct permissions',
  ...
  it.skip(
    'should only show CAM when user has correct permissions',
    { tags: buildTags('@JIRA-STORY:PO-604', '@JIRA-IGNORE') },
    () => {
  ...
  it.skip(
    '(AC.1c)should show CAV and CAM when user has the correct permissions',
    { tags: buildTags('@JIRA-STORY:PO-604', '@JIRA-IGNORE') },
    () => {
      setupComponent(CAM_CAV_DASHBOARD_USER_STATE_MOCK);
