...
describe('Account Enquiry Fixed Penalty', () => {
  ...
  it(
    ...
    () => {
      ...
      let fixedPenaltyDetailsMock = structuredClone(OPAL_FINES_ACCOUNT_DEFENDANT_DETAILS_FIXED_PENALTY_MOCK);
      fixedPenaltyDetailsMock.vehicle_fixed_penalty_flag = true;
      //fixedPenaltyDetailsMock.vehicle_fixed_penalty_details?.notice_number = null;
      //fixedPenaltyDetailsMock.vehicle_fixed_penalty_details.vehicle_registration_number = null;
  ...
  it(
    'AC1a: Company - Fixed Penalty details tab, vehicle fixed penalty, all fields shown',
    { tags: ['@PO-994'] },
    () => {
      let headerMock = structuredClone(createDefendantHeaderMockWithName('Robert', 'Thomson'));
      // TODO: Change to company for other tests, change above to add company name
  ...
  it(
    'AC1b: Company - Fixed Penalty details tab, non-vehicle fixed penalty, partial view',
    { tags: ['@PO-994'] },
    () => {
      let headerMock = structuredClone(createDefendantHeaderMockWithName('Robert', 'Thomson'));
      // TODO: Change to company for other tests, change above to add company name
  ...
  it('AC1c: Company - Fixed Penalty details tab, show em-dash for missing fields', { tags: ['@PO-994'] }, () => {
    let headerMock = structuredClone(createDefendantHeaderMockWithName('Robert', 'Thomson'));
    ...
    cy.get(ACCOUNT_ENQUIRY_FIXED_PENALTY_ELEMENTS.tableTitle)
      ...
      .and('contain.text', 'Fixed Penalty details');
    cy.get(ACCOUNT_ENQUIRY_FIXED_PENALTY_ELEMENTS.labelRegistrationNumber).should('exist').and('contain.text', ' - ');
    cy.get(ACCOUNT_ENQUIRY_FIXED_PENALTY_ELEMENTS.labelTimeOfOffence).should('exist').and('contain.text', ' - ');
  });
});
