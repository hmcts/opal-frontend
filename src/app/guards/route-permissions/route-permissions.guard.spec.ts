...
describe('routePermissionsGuard', () => {
  ...
  const urlPath = FinesRoutingPaths.finesMac;
  ...
  it('should return true if user has permission id', fakeAsync(async () => {
    mockPermissionsService.getUniquePermissions.and.returnValue([FINES_ROUTE_PERMISSIONS[FinesRoutingPaths.finesMac]]);

    const guard = await runRoutePermissionGuard(
      routePermissionsGuard,
      FINES_ROUTE_PERMISSIONS[FinesRoutingPaths.finesMac],
      urlPath,
    );
  ...
  it('should re-route if no access', fakeAsync(async () => {
    ...
    await runRoutePermissionGuard(routePermissionsGuard, FINES_ROUTE_PERMISSIONS[FinesRoutingPaths.finesMac], urlPath);
