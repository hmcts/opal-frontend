...
import { IFinesDraftResolverOptions } from './interfaces/fines-draft-resolver-options.interface';
 ...
 */
export function finesDraftTabResolver(options: IFinesDraftResolverOptions): ResolveFn<IOpalFinesDraftAccountsResponse> {
  return (route: ActivatedRouteSnapshot) => {
    ...
    const params: IOpalFinesDraftAccountParams = buildFinesDraftAccountParams(globalStore.userState(), {
      ...
      includeSubmittedBy: options.includeSubmittedBy,
      includeNotSubmittedBy: options.includeNotSubmittedBy,
    });
