...
describe('buildFinesDraftAccountParams', () => {
  it('should build params scoped to the current user business units', () => {
    ...
    expect(buildFinesDraftAccountParams(OPAL_USER_STATE_MOCK, { statuses })).toEqual({
  ...
  it('should include submittedBy when requested', () => {
    ...
    expect(buildFinesDraftAccountParams(OPAL_USER_STATE_MOCK, { statuses, includeSubmittedBy: true })).toEqual({
      businessUnitIds: OPAL_USER_STATE_MOCK.business_unit_users.map((u) => u.business_unit_id),
      statuses,
  ...
  it('should include notSubmittedBy when requested', () => {
    ...
    expect(buildFinesDraftAccountParams(OPAL_USER_STATE_MOCK, { statuses, includeNotSubmittedBy: true })).toEqual({
      businessUnitIds: OPAL_USER_STATE_MOCK.business_unit_users.map((u) => u.business_unit_id),
      statuses,
