...
describe('defendantAccountHeadingResolver', () => {
  beforeEach(() => {
    ...
    mockFinesService.getDefendantAccountHeadingData.and.returnValue(of(FINES_ACC_DEFENDANT_ACCOUNT_HEADER_MOCK));
  ...
  it('should run resolver pipeline, call service and pass data into account store', async () => {
    ...
    const result = await TestBed.runInInjectionContext(() =>
      firstValueFrom(defendantAccountHeadingResolver(route, state) as Observable<IOpalFinesDefendantAccountHeader>),
    );
    ...
    expect(result).toBe(FINES_ACC_DEFENDANT_ACCOUNT_HEADER_MOCK);
  });
});
