 ...
 */
export function createDefendantDetailsRedirect(router: Router, fragment?: string): RedirectCommand {
  const commands = [FINES_ACC_DEFENDANT_ROUTING_PATHS.children.details];
  const urlTree = fragment ? router.createUrlTree(commands, { fragment }) : router.createUrlTree(commands);
...
/**
 * Creates a redirect command to the major creditor details page.
 * @param router - The Angular Router instance.
 * @returns A RedirectCommand to the major creditor details page.
 */
export function majorCreditorDetailsRedirect(router: Router): RedirectCommand {
  return new RedirectCommand(router.createUrlTree([FINES_ACC_MAJOR_CREDITOR_ROUTING_PATHS.children.details]));
}
