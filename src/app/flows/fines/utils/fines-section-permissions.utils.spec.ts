...
describe('fines-section-permissions.utils', () => {
  ...
  describe('canAccessFinesPrimaryNavigationSection', () => {
    const allReleaseFlagsEnabled = { 'release-1a': true, 'release-1c-write-off': true };
    const release1aDisabled = { 'release-1a': false, 'release-1c-write-off': true };
    const release1cWriteOffDisabled = { 'release-1a': true, 'release-1c-write-off': false };
  ...
  describe('getFeatureFlagReleaseState', () => {
    it('should map raw feature flags into a release feature flag state', () => {
      expect(getFeatureFlagReleaseState({ 'release-1a': true, 'release-1c-write-off': true })).toEqual({
        'release-1a': true,
        'release-1c-write-off': true,
      });
      expect(getFeatureFlagReleaseState({ 'release-1a': false, 'release-1c-write-off': false })).toEqual({
        'release-1a': false,
        'release-1c-write-off': false,
      });
      expect(getFeatureFlagReleaseState({})).toEqual({ 'release-1a': false, 'release-1c-write-off': false });
    });
  });
